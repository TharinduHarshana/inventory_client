...
const EditExpense = () => {
  ...
  useEffect(() => {
    const fetchExpense = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`https://inventory-server-eight.vercel.app/expense/${id}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`https://inventory-server-eight.vercel.app/expense/${id}`, expense);
      navigate("/expense");
    } catch (err) {
      console.error(err);
